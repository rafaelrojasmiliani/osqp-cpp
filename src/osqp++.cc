...
Status<bool> CheckDimensions(const int left_value, const int right_value,
                             std::string left_name, std::string right_name) {
  if (left_value != right_value) {
    return {false, "Dimension mismatch:" + left_name +
                       " (= " + std::to_string(left_value) + ") must equal " +
                       right_name + " (= " + std::to_string(right_value) +
                       ")."};
    ;
  } else {
    return {true, ""};
...
#define OSQP_RETURN_IF_ERROR(expr)                                             \
  {                                                                            \
                                                                               \
    if (!expr.value) {                                                         \
      return Status<bool>{false, expr.message};                                \
    }                                                                          \
    const bool result = expr.value;                                            \
    if (result)                                                                \
      return {result, ""};                                                     \
  }
...
Status<bool> OsqpSolver::Init(const OsqpInstance &instance,
...
Status<bool> VerifySameSparsity(
  ...
  if (new_matrix.nonZeros() != ref_matrix->p[num_variables]) {
    return {false, "The new new matrix should have the same number of non-zero"
                   "elements."};
  }

  for (size_t i = 0; i < num_variables; ++i) {
    if (ref_matrix->p[i] != new_matrix.outerIndexPtr()[i]) {
      return {false, "Sparsity of the new matrix differs from the previously"
                     "defined matrix."};
    }
  }

  for (long i = 0; i < new_matrix.nonZeros(); ++i) {
...
// Helper function for calling osqp_update_P with an upper triangular
// objective matrix. Assumes objective_matrix_upper_triangle is always upper
// triangular.
Status<bool> UpdateUpperTriangularObjectiveMatrix(
  ...
  if (objective_matrix_upper_triangle.rows() !=
    ...
    return {
        false,
        "The new objective matrix should be square with dimension equal to"
        "the "
  ...
  return {false, "Unable to update OSQP P matrix: unrecognized error code."};
}

// Helper function for calling osqp_update_P_A with an upper triangular
// objective matrix. Assumes objective_matrix_upper_triangle is always upper
// triangular.
Status<bool> UpdateUpperTriangularObjectiveMatrixAndConstraintMatrix(
    const Eigen::SparseMatrix<double, Eigen::ColMajor, c_int>
        &objective_matrix_upper_triangle,
    const Eigen::SparseMatrix<double, Eigen::ColMajor, c_int>
        &constraint_matrix,
...
Status<bool> IsUpperTriangular(
  ...
  for (int i = 0; i < matrix.outerSize(); ++i) {
    for (Eigen::SparseMatrix<double, Eigen::ColMajor, c_int>::InnerIterator it(
         ...
         it; ++it) {
      if (it.col() < it.row()) {
        return {false, "number of cols is less than rows"};
      }
    }
  }
  return {true, ""};
...
Status<bool> OsqpSolver::UpdateObjectiveMatrix(
    const Eigen::SparseMatrix<double, Eigen::ColMajor, c_int>
        &objective_matrix) {
...
Status<bool> OsqpSolver::UpdateConstraintMatrix(
    const Eigen::SparseMatrix<double, Eigen::ColMajor, c_int>
        &constraint_matrix) {
  ...
  return {false, "Unable to update OSQP A matrix: unrecognized error code."};
}

Status<bool> OsqpSolver::UpdateObjectiveAndConstraintMatrices(
...
Status<bool> OsqpSolver::SetWarmStart(const Ref<const VectorXd> &primal_vector,
...
Status<bool>
OsqpSolver::SetPrimalWarmStart(const Ref<const VectorXd> &primal_vector) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  const c_int num_variables = workspace_->data->n;
  OSQP_RETURN_IF_ERROR(
  ...
  if (return_code != 0) {
    return {false, "osqp_warm_start_x unexpectedly failed."};
  }
  return {true, ""};
}

Status<bool>
OsqpSolver::SetDualWarmStart(const Ref<const VectorXd> &dual_vector) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  ...
  if (return_code != 0) {
    return {false, "osqp_warm_start_y unexpectedly failed."};
  }
  return {true, ""};
}

Status<bool>
OsqpSolver::SetObjectiveVector(const Ref<const VectorXd> &objective_vector) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  const c_int num_variables = workspace_->data->n;
  ...
  if (return_code != 0) {
    return {false, "osqp_update_lin_cost unexpectedly failed."};
  }
  return {true, ""};
}
...
Status<bool> OsqpSolver::SetBounds(const Ref<const VectorXd> &lower_bounds,
  ...
  for (int i = 0; i < num_constraints; i++) {
    if (lower_bounds[i] > upper_bounds[i]) {
      return {false, "Inconsistent bounds at index " + std::to_string(i) +
  ...
  if (return_code != 0) {
    return {false, "osqp_update_bounds unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<double>> OsqpSolver::GetRho() const {
  if (!IsInitialized()) {
...
Status<double> OsqpSolver::GetSigma() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<c_int> OsqpSolver::GetScaling() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<c_int>> OsqpSolver::GetAdaptiveRho() const {
  if (!IsInitialized()) {
    return {{}, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<c_int>> OsqpSolver::GetAdaptiveRhoInterval() const {
  if (!IsInitialized()) {
    return {{}, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<double>> OsqpSolver::GetAdaptiveRhoTolerance() const {
  if (!IsInitialized()) {
    return {{}, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<double>> OsqpSolver::GetAdaptiveRhoFraction() const {
  if (!IsInitialized()) {
    return {{}, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<c_int>> OsqpSolver::GetMaxIter() const {
  if (!IsInitialized()) {
    return {{}, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<double>> OsqpSolver::GetEpsAbs() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<double>> OsqpSolver::GetEpsRel() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<double>> OsqpSolver::GetEpsPrimInf() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<double>> OsqpSolver::GetEpsDualInf() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<double>> OsqpSolver::GetAlpha() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<double>> OsqpSolver::GetDelta() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<bool>> OsqpSolver::GetPolish() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<c_int>> OsqpSolver::GetPolishRefineIter() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<bool>> OsqpSolver::GetVerbose() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<bool>> OsqpSolver::GetScaledTermination() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<c_int>> OsqpSolver::GetCheckTermination() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<bool>> OsqpSolver::GetWarmStart() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<double>> OsqpSolver::GetTimeLimit() const {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
...
Status<std::optional<bool>> OsqpSolver::UpdateRho(const double rho_new) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  if (rho_new <= 0) {
    return {false, "Invalid rho value: " + std::to_string(rho_new)};
  }
  if (osqp_update_rho(workspace_.get(), rho_new) != 0) {
    return {false, "osqp_update_rho unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<bool>> OsqpSolver::UpdateMaxIter(const int max_iter_new) {
  if (!IsInitialized()) {
    return {{}, "OsqpSolver is not initialized."};
  }
  if (max_iter_new <= 0) {
  ...
  if (osqp_update_max_iter(workspace_.get(), max_iter_new) != 0) {
    return {false, "osqp_update_max_iter unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<bool>> OsqpSolver::UpdateEpsAbs(const double eps_abs_new) {
  if (!IsInitialized()) {
    return {{}, "OsqpSolver is not initialized."};
  }
  if (eps_abs_new < 0.0) {
    return {false, "Invalid eps_abs value: " + std::to_string(eps_abs_new)};
  }
  if (osqp_update_eps_abs(workspace_.get(), eps_abs_new) != 0) {
    return {false, "osqp_update_eps_abs unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<bool>> OsqpSolver::UpdateEpsRel(const double eps_rel_new) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  if (eps_rel_new < 0.0) {
    return {{}, "Invalid eps_rel value: " + std::to_string(eps_rel_new)};
  }
  if (osqp_update_eps_rel(workspace_.get(), eps_rel_new) != 0) {
    return {false, "osqp_update_eps_rel unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<bool>>
OsqpSolver::UpdateEpsPrimInf(const double eps_prim_inf_new) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  if (eps_prim_inf_new < 0.0) {
    return {{},
            "Invalid eps_prim_inf value: " + std::to_string(eps_prim_inf_new)};
  }
  if (osqp_update_eps_prim_inf(workspace_.get(), eps_prim_inf_new) != 0) {
    return {false, "osqp_update_eps_prim_inf unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<bool>>
OsqpSolver::UpdateEpsDualInf(const double eps_dual_inf_new) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  if (eps_dual_inf_new < 0.0) {
    return {{},
            "Invalid eps_dual_inf value: " + std::to_string(eps_dual_inf_new)};
  }
  if (osqp_update_eps_dual_inf(workspace_.get(), eps_dual_inf_new) != 0) {
    return {false, "osqp_update_eps_dual_inf unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<bool>> OsqpSolver::UpdateAlpha(const double alpha_new) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  if (alpha_new <= 0.0 || alpha_new >= 2) {
    return {{}, "Invalid alpha value: " + std::to_string(alpha_new)};
  }
  if (osqp_update_alpha(workspace_.get(), alpha_new) != 0) {
    return {false, "osqp_update_alpha unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<bool>> OsqpSolver::UpdateDelta(const double delta_new) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  if (delta_new <= 0.0) {
    return {{}, "Invalid delta value: " + std::to_string(delta_new)};
  }
  if (osqp_update_delta(workspace_.get(), delta_new) != 0) {
    return {false, "osqp_update_delta unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<bool>> OsqpSolver::UpdatePolish(const bool polish_new) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  if (osqp_update_polish(workspace_.get(), polish_new) != 0) {
    return {false, "osqp_update_polish unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<bool>>
OsqpSolver::UpdatePolishRefineIter(const int polish_refine_iter_new) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  if (polish_refine_iter_new <= 0.0) {
    return {{},
            "Invalid polish_refine_iter value: " +
                std::to_string(polish_refine_iter_new)};
  }
  if (osqp_update_polish_refine_iter(workspace_.get(),
                                     polish_refine_iter_new) != 0) {
    return {{}, "osqp_update_polish_refine_iter unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<bool>> OsqpSolver::UpdateVerbose(const bool verbose_new) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  if (osqp_update_verbose(workspace_.get(), verbose_new) != 0) {
    return {false, "osqp_update_verbose unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<bool>>
OsqpSolver::UpdateScaledTermination(const bool scaled_termination_new) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  if (osqp_update_scaled_termination(workspace_.get(),
                                     scaled_termination_new) != 0) {
    return {{}, "osqp_update_scaled_termination unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<bool>>
OsqpSolver::UpdateCheckTermination(const c_int check_termination_new) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  if (check_termination_new < 0.0) {
    return {{},
            "Invalid check_termination value: " +
                std::to_string(check_termination_new)};
  }
  if (osqp_update_check_termination(workspace_.get(), check_termination_new) !=
      0) {
    return {{}, "osqp_update_check_termination unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<bool>>
OsqpSolver::UpdateWarmStart(const bool warm_start_new) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  if (osqp_update_warm_start(workspace_.get(), warm_start_new) != 0) {
    return {false, "osqp_update_warm_start unexpectedly failed."};
  }
  return {true, ""};
}

Status<std::optional<bool>>
OsqpSolver::UpdateTimeLimit(const double time_limit_new) {
  if (!IsInitialized()) {
    return {false, "OsqpSolver is not initialized."};
  }
  ...
  if (osqp_update_time_limit(workspace_.get(), time_limit_new) != 0) {
    return {false, "osqp_update_time_limit unexpectedly failed."};
  }
  return {true, ""};
}
